...
public class AdapterChatRoom extends BaseQuickAdapter<ChatMsg,BaseViewHolder> {

    public AdapterChatRoom(@Nullable List<ChatMsg> data) {
        super(R.layout.adapter_chat_room,data);
    }

    @Override
    protected void convert(BaseViewHolder helper, ChatMsg item) {
        if (item.getMsgType() == ChatMsg.TYPE_RECEIVED) {
        ...
        } else if (item.getMsgType() == ChatMsg.TYPE_SENT) {
